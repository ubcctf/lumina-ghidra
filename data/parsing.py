...
def extract_md(prog: FlatProgramAPI, func: FunctionDB, gen: Sig) -> dict:
    chunks = []
...
def craft_push_md(prog: FlatProgramAPI, funcs: list[FunctionDB]) -> dict:
    if task:
...
def craft_pull_md(bv: FlatProgramAPI, funcs: list[FunctionDB]) -> list[dict]:
...
def apply_md(bv: FlatProgramAPI, func: FunctionDB, info: Container):
    ...
    for md in info.metadata.serialized_data.chunks:
        if md.type in [MetadataType.MD_INSN_CMT, MetadataType.MD_INSN_REPCMT]:
            ...
            Msg.debug("LuminaPlugin", 'Unimplemented metadata type ' + str(md.type) + ', skipping for now...', logger='Lumina')
